...
$s_fullname = $s_nhiemVu = $s_tienDo = $s_duKien = $s_hoanThanh = $s_user_add = $s_ngayGiao = '';
...
if (!empty($_POST)) {
	...
	$s_ngayGiao = str_replace('\'', '\\\'', $s_ngayGiao);
	...
	if ($s_id != '') {
		//update
		$sql = "update tbl_nhiemvu set fullname = '$s_fullname', nhiemVu = '$s_nhiemVu', ngayGiao = '$s_ngayGiao', tienDo = '$s_tienDo', duKien = '$s_duKien', hoanThanh = '$s_hoanThanh', user_add = '$s_user_add' where id = " .$s_id;
	} else {
		//insert
...
if (isset($_GET['id'])) {
	...
	if ($new != null && count($new) > 0) {
		...
		$s_ngayGiao      = $std['ngayGiao'];
        $s_ngayGiao = date("Y-m-d", strtotime($s_ngayGiao));
		$s_tienDo  = $std['tienDo'];
		$s_duKien = $std['duKien'];
        ...
        <div class="row" id="body-row">
            ...
            <div class="col">
                ...
                <form method="post">
                    ...
                    <div class="form-group">
                        <label for="usr">Ngày giao:</label>
                        <input type="date" class="form-control" id="usr" name="ngayGiao" value="<?=$s_ngayGiao?>">
                    </div>
                    <div class="form-group">
                        <label for="usr">Tiến độ công việc:</label>
                        <input type="text" class="form-control" id="usr" name="tienDo" value="<?=$s_tienDo?>">
                    </div>
                    <div class="form-group">
                        <label for="usr">Lịch hoàn thành:</label>
                        <input type="date" class="form-control" id="usr" name="hoanThanh" value="<?=$s_hoanThanh?>">
                    </div>
                    <div class="form-group">
                        <label for="usr">Dự kiến tháng sau:</label>
                        <input type="text" class="form-control" id="usr" name="duKien" value="<?=$s_duKien?>">
                    </div>
                    <div class="form-group">
